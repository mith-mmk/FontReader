...
- `svg-fonts` 有効時は OpenType `SVG ` glyph を `GlyphLayer::Svg` として保持し、単純 shape は `GlyphLayer::Path` にも変換する
- 現状の対応範囲は `path` / `rect` / `circle` / `ellipse` / `line` / `polyline` / `polygon`、`defs` / `use`、`fill` / `fill-rule` / `stroke` / `stroke-width`、`translate` / `scale` / `matrix`
- gradient や複雑な stroke style、`paintcore` 側の stroke 実描画は未対応
