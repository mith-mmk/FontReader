...
```toml
[dependencies]
fontcore = { version = "0.0.13", features = ["raw"] }
```
...
```toml
[dependencies]
fontcore = { version = "0.0.13", features = ["svg-fonts"] }
```
...
On the 0.0.13 line, the `paintcore` renderer tracks the public `fontcore` layer model closely enough to preserve `clip_commands` and gradient paint values across the `fontcore -> paintcore` conversion.
