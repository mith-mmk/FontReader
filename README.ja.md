...
現在の default feature には `layout` も含まれます。emoji 合字、variation selector、
縦書き置換、RTL shaping などの一般的な GSUB/GPOS shaping は通常ビルドで使えます。
layout なしの軽量構成が必要な場合は `default-features = false` を使ってください。
